...
- Actions (mutually exclusive, return early before the listing path): `--show` (pretty-print a conversation; `cmd_show` collects turns first so `--last N` can slice the tail, `--grep` filters/highlights, `--no-summary` drops `isCompactSummary` turns, `assistant_summary()` collapses tool calls), `--resume` (print, or run with `--exec`), `--set-name`/`--unname` (sidecar names), `--remove`, `--clean` (by age), `--prune-empty` (≤1-prompt junk), `--stats`, `--export` (json/csv). `_resolve_or_exit()` is the shared ID-prefix resolver.
...
- Filters: `--project` matches the directory **basename** (not the full path), `--branch` matches git branch substring, `--since`/`--until` take relative specs (`7d`/`24h`/`2w`) or `YYYY-MM-DD` via `parse_when()` and filter on last activity.
