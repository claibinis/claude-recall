...
- Actions (mutually exclusive, return early before the listing path): `--show` (pretty-print a conversation, `assistant_summary()` collapses tool calls), `--resume` (print, or run with `--exec`), `--set-name`/`--unname` (sidecar names), `--remove`, `--clean` (by age), `--prune-empty` (≤1-prompt junk), `--stats`, `--export` (json/csv). `_resolve_or_exit()` is the shared ID-prefix resolver.
