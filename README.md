...
Claude Code now auto-titles your sessions and lists recent ones in the agent view — but it still can't *search inside* them, and there's no full-text search across your history, no per-session cost or context-window accounting, and no bulk cleanup. `claude-recall` indexes everything in `~/.claude/` and lets you find sessions by keyword (or transcript content), project, date, token usage, or disk size — then read, resume, analyze, or clean them.

### How this relates to native Claude Code

claude-recall is a complement to Claude Code, not a replacement — verified against Claude Code 2.1.x:
